...
allowed_channels = []  # Add your channel IDs here like [-1001234567890]
...
# Command to add allowed channel
    ...
    if message.from_user.id == owner_id:
        try:
            ...
            if channel_id not in allowed_channels:
                ...
                await message.reply("Channel is already authorized.")
        except (IndexError, ValueError):
            await message.reply("Please provide a valid channel ID (with -100 prefix).")
    else:
        await message.reply("You are not authorized to use this command.")

# Function to check authorization
async def is_auth(user_id, chat_id):
...
async def txt_handler(bot: Client, m: Message):
    ...
    if not await is_auth(user_id, chat_id):
...
# Text handler for both private and channel messages
@bot.on_message(filters.text & (filters.private | filters.channel))
async def text_handler(bot: Client, m: Message):
    # Skip if message is from bot
    if m.from_user and m.from_user.is_bot:
        return
    
    # Check authorization for private messages
    if m.chat.type == "private":
        user_id = m.from_user.id
        if not await is_auth(user_id, m.chat.id):
            return
    
    # Check if message is from allowed channel
    if m.chat.type == "channel":
        if m.chat.id not in allowed_channels:
            return
    ...
    input2: Message = await bot.listen(editable.chat.id, filters=filters.text)
    raw_text2 = input2.text
    quality = input2.text
    await input2.delete(True)
    try:
    ...
    input4: Message = await bot.listen(editable.chat.id, filters=filters.text)
    raw_text4 = input4.text
    await input4.delete(True)
    if raw_text4 == '?':
        PW = pw_token
    ...
    input6 = message = await bot.listen(editable.chat.id, filters=filters.text)
    raw_text6 = input6.text
    await input6.delete(True)
    await editable.delete()
